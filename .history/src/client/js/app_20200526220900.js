...
submitForm = async () => {
    ...
    try {
        ...
        const dayDiff = Math.ceil((departureDate.getTime() - Date.now()) / (1000 * oneDaySeconds))
        updateUI(data.cityFrom, data.cityTo, weatherData, departureDate, dayDiff)
    } catch (error) {
...
const updateUI = (cityFrom, cityTo, weatherData, date, dayDiff) => {
    console.log(`Your're going from ${cityFrom} to ${cityTo} in ${dayDiff} days, and the weather's gonna be like ${weatherData.weather.description}, with the temperature going as high as ${weatherData.high_temp}`)
}
